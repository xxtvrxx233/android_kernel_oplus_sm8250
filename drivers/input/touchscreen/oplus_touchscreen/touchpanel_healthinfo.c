...
int tp_healthinfo_report(void *tp_monitor_data, healthinfo_type type, void *value)
{
	...
	case HEALTH_TOUCH:
		if (monitor_data->touch_points) {
			ret = tp_touch_healthinfo_handle(monitor_data, *value_int, monitor_data->touch_points,
											monitor_data->touch_num, monitor_data->max_finger_support, monitor_data->direction);
